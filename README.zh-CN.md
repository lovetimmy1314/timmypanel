...
- 明暗主题，图片 / 纯色 / 渐变壁纸
- 电脑端左上角悬浮天气（可在设置里关掉，手机上不显示）：选城市或区，或用浏览器定位，默认走 Open-Meteo（免 key），也可在天气设置里配和风天气以便精确到区；天气由服务端代取并缓存，浏览器不直连第三方
- 电脑端右上角悬浮万年历（可在设置里关掉，手机上不显示）：农历、二十四节气、传统节日，展开是整月面板；有当年国务院放假通知数据的年份还会标出放假和补班
- 电脑端右侧悬浮快捷访问卡片（可在设置里关掉，手机上不显示）：快速直达高频常用网站，卡片仅保留图标与名称，悬停显示完整描述，支持从已收录网站勾选与排序
- 中英双语，适配手机，可添加到主屏幕
...
backup:
  auto_daily: true
  keep: 7
weather:                         # 可选的实例级回落；每个账号也可以在天气设置里自己配和风
