...
## 提交与版本：改完就提交，代码改动打 tag，不用问

**这个仓库的既定要求是每次执行完后就进行 `git commit` 一次，用中文写 message**，不需要每次征求同意（这一条覆盖「只在用户要求时才提交」的默认行为）。
- **只有修改代码才打 tag**：当本次提交包含代码改动（Go、Vue/TS/CSS 等应用代码）时，提交后**必须打上递增的迭代 tag 版本号**（遵循 SemVer 规范，格式 `vMAJOR.MINOR.PATCH`，常规代码改动 patch +1，如 `v1.0.0` → `v1.0.1`）。
- **纯文件与落盘改动不打 tag**：若仅修改文档、落盘文件（`docs/plans.md`、`docs/decisions.md`、`docs/conventions.md`、`README` 等），**只提交不打 tag**。
- 提交前先跑通检查，格式和粒度见 `docs/conventions.md` 的 Git 一节。
- 直接提交到 `main`，不开分支，不 push 到远端除非人明确要求。
