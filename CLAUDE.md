...
## 提交与版本：改完就提交并打 tag，不用问

**这个仓库的既定要求是每次执行完代码改动后，就进行`git commit` 一次，用中文写 message，且提交后必须打上递增的迭代 tag 版本号**，
不需要每次征求同意（这一条覆盖「只在用户要求时才提交」的默认行为）。
- tag 遵循 SemVer 规范（格式 `vMAJOR.MINOR.PATCH`，常规改动 patch +1，如 `v1.0.0` → `v1.0.1`），用于精细化版本管理与回溯。
