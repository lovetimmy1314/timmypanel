...
标签页、每次刷新都是一次出站。缓存按坐标量化到 0.1°（约 11km）做键，TTL 10 分钟，
真正发给上游的也是这个量化后的坐标（顺带少给一点精度）。
...
- 代价三：城市搜索的关键词是用户随手敲的，等于一个「登录后可用的出站放大器」。
...
国务院办公厅单独发通知，没有规律可推。当前只录了 2024、2025 两年。

- 代价：当年通知没录进去之前，那一年的日历只有节日名，没有班/休。这是刻意的——
  拿去年的表猜今年会给出确凿的错误信息，比不显示更糟。加新一年时照抄通知原文。
