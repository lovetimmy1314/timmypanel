...
- **应用内一键换镜像 / 替换自身二进制**：容器里没有 docker.sock，systemd 下二进制只读。
  关于页只检测并复制 `update.sh`，真正升级仍在宿主机（决策 030、037）。
...
天气组件的上游（默认 `api.open-meteo.com`，配了和风则是 `*.qweatherapi.com`）也走这条路，连不上就一直显示 `--`。
按「部署在能直连目标站点的机器上」来解决；真要加代理支持，注意它和决策 003 的冲突
（SSRF 防护是挂在 `DialContext` 上的，走了代理就等于把选址权交给代理，见决策 016 的「代价」）。

**法定调休只录到 2026 年。** 表在 `internal/service/holidays.go`，按年写死，
数据来自当年的国务院办公厅通知（2024 是国办发明电〔2023〕7 号，2025 是〔2024〕8 号，
2026 是〔2025〕7 号）。没有数据的年份**不显示班/休角标**，只显示节日名（决策 034）
——这是刻意的，不是漏了。每年 11 月通知发布后往那张表里加一段即可：照抄原文，
别从「一般规律」推，`TestHolidayTableWellFormed` 会挡住抄坏的条目。
...
| 设置里搜城市选一条再保存 | 库里的 `lat/lon` 被截到两位小数，卡片立刻换成新地点的天气 |
| 设置里搜「海淀」选第一条再保存 | 显示名带区（如「海淀·北京市」），不是只到「北京市」 |
| 搜一个 Open-Meteo 没有的区（如「天河区」） | 未配和风时显示「没搜到」，**不会**悄悄变成所属市 |
| 天气设置里改用和风，填齐合法 host+key 再搜「天河区」 | 能出「天河」这类区级结果；网络面板过滤 `qweather` **一条浏览器请求都没有** |
| 只填了 host 或 key、或 host 写成别人的域名 | 保存后回落到 Open-Meteo，不 502 |
| 点左上角天气药丸 | 展开详情卡，**不**打开设置中心；详情底部「设置」才跳过去 |
| 切到和风后点开详情 | 出现紫外线 / 能见度 / 气压 / 日出日落；有预警才显示那一行 |
| 切回 Open-Meteo 再点开 | 那些和风行消失，仍是体感 / 今日 / 湿度 / 风速 |
...
| 未登录打 `/api/v1/update/check` | 401 |
| `GET /api/v1/auth/config` | JSON 里有 `version`（开发构建是 `dev`），HEALTHCHECK 仍 200 |
| 关于页 | 显示当前版本；点「检测更新」才打 `/update/check`，打开面板不出站 |
| 非管理员点出有新版本 | 能看到提示，**没有**复制命令那一块 |
| 管理员 + Docker 部署 + 有新版本 | 出现三条 `update.sh` 命令，复制后和 README 升级段一字不差 |
| 管理员 + 非 Docker + 有新版本 | 提示按文档换二进制，不给 `update.sh` |
| `go run` 的 dev 构建点检测 | 提示开发构建，不判断新旧 |
| 断网或 GitHub 不通 | 检测返回 `upstreamUnavailable`，**不** 502；一小时内再点不重复出站（失败缓存 5 分钟） |
