...
| `config.yaml` 配齐合法的 `qweather_host` + `qweather_key` 后重启，再搜「天河区」 | 能出「天河」这类区级结果；网络面板过滤 `qweather` **一条浏览器请求都没有** |
| 只配了 host 或 key、或 host 写成别人的域名 | 日志有 WARN，天气仍走 Open-Meteo，不 502 |
| `weather.provider: open-meteo` 同时配了和风 key | 强制走 Open-Meteo |
